...
export default function LoginPage({ onSwitch, onSuccess }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!res.ok) { setError(data.detail || 'שגיאה בהתחברות'); return }
  ...
  async function handleGoogle(credential) {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) { setError(data.detail || 'שגיאה בכניסה עם גוגל'); return }
      login(data.access_token, data.user)
      onSuccess?.()
    } catch {
      setError('שגיאת רשת — נסה שוב')
  ...
  return (
    <div style={styles.card}>
      <h2 style={styles.title}>כניסה לחשבון</h2>
      ...
      <p style={styles.switch}>
        אין לך חשבון?{' '}
        <button onClick={onSwitch} style={styles.link}>הירשם כאן</button>
      </p>
    </div>
  )
}
...
const styles = {
  ...
  divider: {
    ...
    alignItems: 'center',
    gap: 10,
    '::before': { content: '""', flex: 1, height: 1, background: '#e5e7eb' },
  },
